...
public class GoC5 extends SearchAction {
   @Override
   public SearchBasedAgentState execute(SearchBasedAgentState s) {
        CoronaTrackerAgentState agentState = (CoronaTrackerAgentState) s;
       if (agentState.getVisitedPositions().contains(CoronaTrackerAgentState.C5)) {
          return null;
      }
      ...
      if (successors != null) {
          ...
          if (index >= 0 && !agentState.getCaminosCortados().contains(new Camino(agentState.getPosition(),CoronaTrackerAgentState.C5))) {
               agentState.setPosition(CoronaTrackerAgentState.C5);
               agentState.setCostCount(agentState.getCostCount() + this.getCost());
//               System.out.println("working... C5");
               return agentState;
         }
   }
     return null;
