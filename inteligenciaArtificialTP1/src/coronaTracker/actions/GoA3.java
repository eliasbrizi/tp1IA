...
public class GoA3 extends SearchAction {
   @Override
   public SearchBasedAgentState execute(SearchBasedAgentState s) {
        CoronaTrackerAgentState agentState = (CoronaTrackerAgentState) s;
       if (agentState.getVisitedPositions().contains(CoronaTrackerAgentState.A3)) {
          return null;
      }
      ...
      if (successors != null) {
          ...
          if (index >= 0 && !agentState.getCaminosCortados().contains(new Camino(agentState.getPosition(),CoronaTrackerAgentState.A3))) {
               agentState.setPosition(CoronaTrackerAgentState.A3);
               agentState.setCostCount(agentState.getCostCount() + this.getCost());
//               System.out.println("working... A3");
               return agentState;
         }
   }
     return null;
